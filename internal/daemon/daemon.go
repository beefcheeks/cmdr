...
func registerAPI(mux *http.ServeMux, s *scheduler.Scheduler, bus *EventBus, database *sql.DB) {
	...
	mux.HandleFunc("/api/squads/amend", handleAmend(database, bus))
	...
	mux.HandleFunc("/api/agent/tasks/rerun", handleRerunTask(database, bus))
	mux.HandleFunc("/api/agent/tasks/refresh-design", handleRefreshDesign(database, bus))
	mux.HandleFunc("/api/agent/tasks/revise", handleReviseTask(database, bus))
